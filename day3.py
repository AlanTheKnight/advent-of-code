...
didn't quite follow the packing instructions, and so a few
...
number of items in each of its two compartments, so the
first half of the characters represent items in the first
...
alphabet = {letter: index + 1 for index, letter in enumerate(ascii_letters)}
...
#         rucksach = line.strip()
#         mid = len(rucksach) // 2
#         shared = set(rucksach[:mid]) & set(rucksach[mid:])
...
For efficiency, within each group of three Elves, the
badge is the only item type carried by all three Elves.
That is, if a group's badge is item type B, then all three
Elves will have item type B somewhere in their rucksack,
and at most two of the Elves will be carrying any other
item type.

The problem is that someone forgot to put this year's
updated authenticity sticker on the badges. All of the
badges need to be pulled out of the rucksacks so the new
authenticity stickers can be attached.

Additionally, nobody wrote down which item type corresponds
to each group's badges. The only way to tell which item
type is the right one is by finding the one item type that
is common between all three Elves in each group.

Every set of three lines in your list corresponds to a
single group, but each group can have a different badge
item type. So, in the above example, the first group's
...
with open("day3.input.txt", "r") as f:
    ...
    for l_i, l in enumerate(f.readlines()):
        cur_group.append(l.strip())
        if (l_i + 1) % 3 == 0:
            a, b, c = map(set, cur_group)
            badge = a & b & c
            for letter in badge:
                ans += alphabet[letter]
            cur_group = []
